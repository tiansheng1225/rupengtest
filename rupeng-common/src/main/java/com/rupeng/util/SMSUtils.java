...
public class SMSUtils {
    ...
    //返回1表示验证成功；返回2表示验证失败，但还可以继续验证；返回3表示验证失败并且不能继续验证，需要重新发送邮件验证码
    public static int checkSMSCode(HttpSession session, String phone, String requestCode) {
        ...
        if (!requestCode.equalsIgnoreCase(sessionCode)) {
            ...
            if (remainCount <= 0) {
                ...
                logger.debug("检查邮件验证码失败，验证码错误，phone：{}，requestCode：{}，sessionCode：{}，remainCount：{}", phone, requestCode, sessionCode, remainCount);
                return CHECK_RESULT_INVALID;
            } else {
        ...
        logger.debug("检查邮件验证码成功，phone：{}，requestCode：{}，sessionCode：{}，remainCount：{}", phone, requestCode, sessionCode, remainCount);
        return CHECK_RESULT_TRUE;
    }

    public static void sendSMSCode(HttpSession session, String url, String username, String appKey, String template, String phone) {
        // 得到一个随机的5位数字的验证码
